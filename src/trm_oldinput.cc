...
Subs::Input::Input(const std::string& global_file, const std::string& local_file, int argc, char *argv[]) 
  ...
  vsi = find_if(simple_args.begin(), simple_args.end(), Check_all_arg("nodefs"));
  if(vsi != simple_args.end())
    throw Input_Error("Cannot repeat keyword 'initialise' in argument list");
...
void Subs::Input::get_value(std::string name, std::string& value, const std::string& initial_value, const std::string& prompt_string){
  ...
  if(vsi == named_args.end()){
    ...
    if(mp->second.prompt == PROMPT && narg < simple_args.size()){
      ...
      if(simple_args[narg] == "\\"){
	value = initial_value;
	Header::const_iterator hci;
	if(mp->second.store_where == LOCAL){
	  hci = local_defs.find(name);
	  if(hci != local_defs.end()) value = hci->second->get_string();
	}else{
	  hci = global_defs.find(name);
	  if(hci != global_defs.end()) value = hci->second->get_string();
	}
	set_to_default = true;
	...
	value = simple_args[narg];
      ...
      if(mp->second.store_where == LOCAL){
	hci = local_defs.find(name);
	if(hci != local_defs.end()) default_value = hci->second->get_string();
      }else{
	hci = global_defs.find(name);
	if(hci != global_defs.end()) default_value = hci->second->get_string();
      }
...
void Subs::Input::get_value(std::string name, bool& value, const bool& initial_value, const std::string& prompt_string){
  ...
  if(vsi == named_args.end()){
	...
	if(mp->second.store_where == LOCAL){
	  hci = local_defs.find(name);
	  if(hci != local_defs.end()) hci->second->get_value(value);
	}else{
	  hci = global_defs.find(name);
	  if(hci != global_defs.end()) hci->second->get_value(value);
	}
	set_to_default = true;
	...
	throw Input_Error(std::string("Error translating ") + simple_args[narg] + std::string(" for input = ") + name);
      ...
      if(mp->second.store_where == LOCAL){
	hci = local_defs.find(name);
	if(hci != local_defs.end()) default_value = hci->second->get_bool();
      }else{
	hci = global_defs.find(name);
	if(hci != global_defs.end()) default_value = hci->second->get_bool();
      }
...
void Subs::Input::get_value(std::string name, char& value, const char& initial_value, const std::string& allowed_values, const std::string& prompt_string){
  ...
  if(vsi == named_args.end()){
	...
	if(mp->second.store_where == LOCAL){
	  hci = local_defs.find(name);
	  if(hci != local_defs.end()) value = hci->second->get_char();
	}else{
	  hci = global_defs.find(name);
	  if(hci != global_defs.end()) value = hci->second->get_char();
	}
	...
	if(!istr) throw Input_Error(std::string("Error translating ") + simple_args[narg] + std::string(" for input = ") + name);
      ...
      if(mp->second.store_where == LOCAL){
	hci = local_defs.find(name);
	if(hci != local_defs.end()) default_value = hci->second->get_char();
      }else{
	hci = global_defs.find(name);
	if(hci != global_defs.end()) default_value = hci->second->get_char();
      }
...
void Subs::Input::get_value(std::string name, std::vector<double>& value, const double& initial_value, 
  ...
  if(vsi == named_args.end()){
	...
	value = temp;
      ...
      if(mp->second.store_where == LOCAL){
	hci = local_defs.find(name);
	if(hci != local_defs.end()) hci->second->get_value(default_value);
      }else{
	hci = global_defs.find(name);
	if(hci != global_defs.end()) hci->second->get_value(default_value);
      }
...
bool Subs::Input::is_a_global(const std::string& name) const {
  return (global_defs.find(name) != global_defs.end());
}
